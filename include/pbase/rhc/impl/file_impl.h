...
static size_t file_stream_read(Stream_i stream, void *msg, size_t size) {
    ...
    if(n <= 0) {
        log_error("p_rhc_file_read failed, killing file...");
...
static size_t file_stream_write(Stream_i stream, void *msg, size_t size) {
    ...
    if(n <= 0) {
        log_error("p_rhc_file_write failed, killing file...");
        SDL_RWclose(impl->file);
        impl->file = NULL;
        return 0;
    }
...
RhcFile *p_rhc_file_open_read_a(const char *file, bool ascii, Allocator_i a) {
    ...
    if (!impl->file) {
        p_rhc_error = "p_rhc_file_open_read_a failed";
        log_warn("p_rhc_file_open_read_a failed: %s", file);
        a.free(a, self);
        return NULL;
    }
    return self;
...
RhcFile *p_rhc_file_open_write_a(const char *file, bool ascii, Allocator_i a) {
    ...
    if (!impl->file) {
        p_rhc_error = "p_rhc_file_open_write_a failed";
        log_warn("p_rhc_file_open_write_a failed: %s", file);
        a.free(a, self);
        return NULL;
    }
    return self;
...
RhcFile *p_rhc_file_open_append_a(const char *file, bool ascii, Allocator_i a) {
    ...
    if (!impl->file) {
        p_rhc_error = "p_rhc_file_open_append_a failed";
        log_warn("p_rhc_file_open_append_a failed: %s", file);
        a.free(a, self);
        return NULL;
    }
    return self;

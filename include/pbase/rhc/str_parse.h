...
// helper functions:
static bool p_rhc_str__helper_system_is_binary_little_endian_() {
    int n = 1;
    // little endian if true
    return *(char *) &n == 1;
}
static void p_rhc_str__helper_swap_endian_(void *buf, int n) {
    assert(n <= 256 && "p_rhc_str__helper_swap_endian_: max 256 bits");
    char tmp[256];
...
static Str_s str_eat_int ## x ## _ascii(Str_s s, int ## x ## _t *opt_eaten) {\
    ...
    if(ate_size <= 0 || ate_size > s.size) {\
        p_rhc_error = "str_eat_int_ascii failed";\
...
static Str_s str_eat_uint ## x ## _ascii(Str_s s, uint ## x ## _t *opt_eaten) {\
    ...
    size_t ate_size = end - s.data;\
    if(ate_size <= 0 || ate_size > s.size) {\
        p_rhc_error = "str_eat_uint_ascii failed";\
...
static Str_s str_eat_float_ascii(Str_s s, float *opt_eaten) {
    ...
    size_t ate_size = end - s.data;
    if(ate_size <= 0 || ate_size > s.size) {
        p_rhc_error = "str_eat_float_ascii failed";
...
static Str_s str_eat_double_ascii(Str_s s, double *opt_eaten) {
    ...
    size_t ate_size = end - s.data;
    if(ate_size <= 0 || ate_size > s.size) {
        p_rhc_error = "str_eat_double_ascii failed";
...
static Str_s str_eat_int ## x ## _binary_le(Str_s s, int ## x ## _t *opt_eaten) {\
    if(str_empty(s))\
        return s;\
    if(s.size < (x)/8) {\
        p_rhc_error = "str_eat_int_binary_le failed";\
        log_warn("str_eat_int" #x "_binary_le: failed");\
        return str_new_invalid();\
    }\
    if(opt_eaten){\
        memcpy(opt_eaten, s.data, (x)/8);\
        if(!p_rhc_str__helper_system_is_binary_little_endian_())\
            p_rhc_str__helper_swap_endian_(opt_eaten, (x)/8);\
    }\
    s.data += (x)/8;\
    s.size -= (x)/8;\
    return s;\
...
static Str_s str_eat_int ## x ## _binary_be(Str_s s, int ## x ## _t *opt_eaten) {\
    if(str_empty(s))\
        return s;\
    if(s.size < (x)/8) {\
        p_rhc_error = "str_eat_int_binary_be failed";\
...
static Str_s str_eat_uint ## x ## _binary_le(Str_s s, uint ## x ## _t *opt_eaten) {\
    if(str_empty(s))\
        return s;\
    if(s.size < (x)/8) {\
        p_rhc_error = "str_eat_uint_binary_le failed";\
...
static Str_s str_eat_uint ## x ## _binary_be(Str_s s, uint ## x ## _t *opt_eaten) {\
    if(str_empty(s))\
        return s;\
    if(s.size < (x)/8) {\
        p_rhc_error = "str_eat_uint_binary_be failed";\
...
static Str_s str_eat_float_binary_le(Str_s s, float *opt_eaten) {
    if(str_empty(s))
        return s;
    if(s.size < 4) {
        p_rhc_error = "str_eat_float_binary_le failed";
    ...
    if(opt_eaten) {
        ...
        if (!p_rhc_str__helper_system_is_binary_little_endian_())
            p_rhc_str__helper_swap_endian_(opt_eaten, 4);
    }
    s.data += 4;
    s.size -= 4;
    return s;
...
static Str_s str_eat_float_binary_be(Str_s s, float *opt_eaten) {
    if(str_empty(s))
        return s;
    if(s.size < 4) {
        p_rhc_error = "str_eat_float_binary_be failed";
...
static Str_s str_eat_double_binary_le(Str_s s, double *opt_eaten) {
    if(str_empty(s))
        return s;
    if(s.size < 8) {
        p_rhc_error = "str_eat_double_binary_le failed";
    ...
    if(opt_eaten) {
        ...
        if (!p_rhc_str__helper_system_is_binary_little_endian_())
            p_rhc_str__helper_swap_endian_(opt_eaten, 8);
    }
    s.data += 8;
    s.size -= 8;
    return s;
...
static Str_s str_eat_double_binary_be(Str_s s, double *opt_eaten) {
    if(str_empty(s))
        return s;
    if(s.size < 8) {
        p_rhc_error = "str_eat_double_binary_be failed";
...
static Str_s str_feed_int ## x ## _binary_le(Str_s s, int ## x ## _t feed) {\
    if(str_empty(s))\
        return s;\
    if(s.size < (x)/8) {\
        p_rhc_error = "str_feed_int_binary_le failed";\
        log_warn("str_feed_int" #x "_binary_le: failed");\
        return str_new_invalid();\
    }\
    if(!p_rhc_str__helper_system_is_binary_little_endian_())\
        p_rhc_str__helper_swap_endian_(&feed, (x)/8);\
...
static Str_s str_feed_int ## x ## _binary_be(Str_s s, int ## x ## _t feed) {\
    if(str_empty(s))\
        return s;\
    if(s.size < (x)/8) {\
        p_rhc_error = "str_feed_int_binary_be failed";\
...
static Str_s str_feed_uint ## x ## _binary_le(Str_s s, uint ## x ## _t feed) {\
    if(str_empty(s))\
        return s;\
    if(s.size < (x)/8) {\
        p_rhc_error = "str_feed_uint_binary_le failed";\
...
static Str_s str_feed_uint ## x ## _binary_be(Str_s s, uint ## x ## _t feed) {\
    if(str_empty(s))\
        return s;\
    if(s.size < (x)/8) {\
        p_rhc_error = "str_feed_uint_binary_be failed";\
...
static Str_s str_feed_float_binary_le(Str_s s, float feed) {
    if(str_empty(s))
        return s;
    if(s.size < 4) {
        p_rhc_error = "str_feed_float_binary_le failed";
        log_warn("str_feed_float_binary_le: failed");
        return str_new_invalid();
    }
    if (!p_rhc_str__helper_system_is_binary_little_endian_())
        p_rhc_str__helper_swap_endian_(&feed, 4);
...
static Str_s str_feed_float_binary_be(Str_s s, float feed) {
    if(str_empty(s))
        return s;
    if(s.size < 4) {
        p_rhc_error = "str_feed_float_binary_be failed";
...
static Str_s str_feed_double_binary_le(Str_s s, double feed) {
    if(str_empty(s))
        return s;
    if(s.size < 8) {
        p_rhc_error = "str_feed_double_binary_le failed";
        log_warn("str_feed_double_binary_le: failed");
        return str_new_invalid();
    }
    if (!p_rhc_str__helper_system_is_binary_little_endian_())
        p_rhc_str__helper_swap_endian_(&feed, 8);
...
static Str_s str_feed_double_binary_be(Str_s s, double feed) {
    if(str_empty(s))
        return s;
    if(s.size < 8) {
        p_rhc_error = "str_feed_double_binary_be failed";

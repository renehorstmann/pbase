...
def cast_dmat2_np(data: dmat2) -> np.ndarray:
    return np.ctypeslib.as_array(ct.cast(ct.pointer(data), c_double_p), shape=(2, 2))

def cast_np_dmat2_p(data: np.ndarray) -> dmat2_p:
    if data is None:
        data = np.zeros(4, dtype=np.float64)
    else:
        if data.dtype != np.float64:
            raise RuntimeError('cast_np_dmat2_p failed, wrong type')
        if data.ndim != 2 or data.shape[0] != 2 or data.shape[1] != 2:
            raise RuntimeError('cast_np_dmat2_p failed, wrong size')
        if np.isfortran(data):
            raise RuntimeError('cast_np_dmat2_p failed: must be C order')
    return data.ctypes.data_as(dmat2_p)

def cast_np_dmat2(data: np.ndarray) -> dmat2:
    return cast_np_dmat2_p(data).contents

def cast_array_dmat2(data) -> dmat2:
    if data is None:
        data = np.zeros(4, dtype=np.float64)
    return cast_np_dmat2(np.float64(data))


def cast_dmat3_np(data: dmat3) -> np.ndarray:
    return np.ctypeslib.as_array(ct.cast(ct.pointer(data), c_double_p), shape=(3, 3))

def cast_np_dmat3_p(data: np.ndarray) -> dmat3_p:
    if data is None:
        data = np.zeros(3*3, dtype=np.float64)
    else:
        if data.dtype != np.float64:
            raise RuntimeError('cast_np_dmat3_p failed, wrong type')
        if data.ndim != 2 or data.shape[0] != 3 or data.shape[1] != 3:
            raise RuntimeError('cast_np_dmat3_p failed, wrong size')
        if np.isfortran(data):
            raise RuntimeError('cast_np_dmat3_p failed: must be C order')
    return data.ctypes.data_as(dmat3_p)

def cast_np_dmat3(data: np.ndarray) -> dmat3:
    return cast_np_dmat3_p(data).contents

def cast_array_dmat3(data) -> dmat3:
    if data is None:
        data = np.zeros(3*3, dtype=np.float64)
    return cast_np_dmat3(np.float64(data))


def cast_dmat4_np(data: dmat4) -> np.ndarray:
    return np.ctypeslib.as_array(ct.cast(ct.pointer(data), c_double_p), shape=(4, 4))

def cast_np_dmat4_p(data: np.ndarray) -> dmat4_p:
    if data is None:
        data = np.zeros(4*4, dtype=np.float64)
    else:
        if data.dtype != np.float64:
            raise RuntimeError('cast_np_dmat4_p failed, wrong type')
        if data.ndim != 2 or data.shape[0] != 4 or data.shape[1] != 4:
            raise RuntimeError('cast_np_dmat4_p failed, wrong size')
        if np.isfortran(data):
            raise RuntimeError('cast_np_dmat4_p failed: must be C order')
    return data.ctypes.data_as(dmat4_p)

def cast_np_dmat4(data: np.ndarray) -> dmat4:
    return cast_np_dmat4_p(data).contents

def cast_array_dmat4(data) -> dmat4:
    if data is None:
        data = np.zeros(4*4, dtype=np.float64)
    return cast_np_dmat4(np.float64(data))
